...
4.  **强制编译验证 (Mandatory Compilation Validation)**: 每次代码改动完成后，**必须**立即执行对应平台的代码编译命令（如 `./gradlew :composeApp:compileKotlinJvm`），以实证方式确保修改没有引入任何编译错误。**验证是任务完成的唯一标准，严禁在未确认编译通过的情况下交付任务。**
...
- **编译优先 (Compiling First)**: **【红线规则】任何涉及代码逻辑或文件结构的物理改动后，必须优先进行编译校验。只有编译通过才可认为当前 Directive 执行成功。若编译失败，必须立即修正直至通过，不得带病进入下一阶段。**
